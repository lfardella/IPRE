...
data = read('../../Datos/2017/C1.MT12.HH*.2017.356-360.resampled.mseed')
signal = data[0].data
stats = data[0].stats
...
# Crear un eje de tiempo en días (para considerar 5 días)
...
# Convertir los índices filtrados a tiempo en días con un umbral de 0.75
filter_mask = profile > 0.75
...
print(f"Detecciones con perfil > 0.75: {len(filtered_time_profile)}")

# Extraer la fecha inicial y final del archivo
start_year, start_day_of_year = 2017, 356
...
axes[1].axhline(y=0.75, color='red', linestyle='--', linewidth=1, label="Umbral 0.75")
...
axes[2].set_ylim(0, 5)  # Ajustar límites del eje Y para 5 días
...
plt.savefig("results_plot_5days_075.png", dpi=300, bbox_inches='tight')
plt.show()
